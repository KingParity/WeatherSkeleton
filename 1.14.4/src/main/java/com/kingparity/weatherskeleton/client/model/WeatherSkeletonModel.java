...
@OnlyIn(Dist.CLIENT)
public class WeatherSkeletonModel<T extends WeatherSkeletonEntity> extends EntityModel<T> implements IHasArm, IHasHead
{
    ...
    public BipedModel.ArmPose left_arm_pose = BipedModel.ArmPose.EMPTY;;
    public BipedModel.ArmPose right_arm_pose = BipedModel.ArmPose.EMPTY;;
    ...
    public void setRotationAngles(T weatherSkeleton, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scaleFactor)
    {
        ...
        if (this.swingProgress > 0.0F)
        {
            ...
            RendererModel renderer = this.getArm(handSide);
    ...
    {
        this.getArm(side).postRender(scale);
    }
    
    private RendererModel getArm(HandSide side)
    {
    ...
    private HandSide getMainHand(T weatherSkeleton)
    {
        HandSide handside = weatherSkeleton.getPrimaryHand();
        return weatherSkeleton.swingingHand == Hand.MAIN_HAND ? handside : handside.opposite();
    }
    ...
    public void setModelAttributes(EntityModel<T> model)
    {
        ...
        if(model instanceof WeatherSkeletonModel)
        {
            ...
            weatherSkeletonModel.left_arm_pose = this.left_arm_pose;
            weatherSkeletonModel.right_arm_pose = this.right_arm_pose;
